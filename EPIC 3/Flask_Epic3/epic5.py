...
if campaign_id_to_find_users_for not in df['Campagne'].unique():
    print(f"Campaign {campaign_id_to_find_users_for} not found in the dataset.")
else:
    ...
    if precision is not None:
        ...
        with open(f'similar_users_{campaign_id_to_find_users_for}.txt', 'w') as file:
            file.write(f"Users Similar to Campaign {campaign_id_to_find_users_for} (Not Attended):\n")
            for i, (user_index, similarity) in enumerate(similar_users_to_campaign, 1):
                user_id = df.iloc[user_index]['Conctactpersoon']
                file.write(f"{i}. User ID: {user_id}, Similarity: {similarity*100:.2f}%\n")

        print(f"Similar users information saved to 'similar_users_{campaign_id_to_find_users_for}.txt'")
