...
def get_recommendations_route():
    ...
    actual_campaigns = set(df[df['Contactfiche'] == user_id]['Campagne'])
    recommended_campaigns = set(campaign_id for _, _, campaign_id, _ in filtered_recommendations)
    
    attended_campaigns = df[df['Contactfiche'] == user_id].index.tolist()
...
def download_excel(user_id):
    txt_file_path = f'user_recommendations_{user_id}.txt'  # Update the file path accordingly
